...
$fan = New-Fixture 29 'DELL EC: Example System' 'CPU' '2529' '2,529 RPM'
$motherboardFan = New-Fixture 30 'DELL EC: Example System' 'Mainboard' '1049' '1,049 RPM'
...
Assert-Fixture ($fan.Kind -eq 'FAN' -and $fan.Scope -eq 'AGGREGATE' -and $fan.Unit -eq 'RPM') 'exact CPU fan RPM export accepted'
Assert-Fixture ((Select-CPUExport @($fan) 'FAN' -1) -match '^FAN\|HKEY_CURRENT_USER\|29\|.*\|RPM$') 'automatic CPU fan export uses FAN protocol'
Assert-Fixture ($motherboardFan.Kind -eq 'MBFAN' -and $motherboardFan.Unit -eq 'RPM') 'exact motherboard fan RPM export accepted'
Assert-Fixture ((Select-CPUExport @($motherboardFan) 'MBFAN' -1) -match '^MBFAN\|HKEY_CURRENT_USER\|30\|.*\|RPM$') 'automatic motherboard fan export uses MBFAN protocol'
Assert-Fixture ($null -eq (New-Fixture 31 'dGPU [#1]: Example' 'GPU Fan' '1252' '1,252 RPM') -and
    $null -eq (New-Fixture 32 'Example Board' 'Pump' '2200' '2,200 RPM')) 'non-CPU fans rejected'
