...
function suite.Run(settingsPath)
    ...
    test('dependent details collapse, preserve saved values and restore after visibility returns', function()
        local c = newMock(settingsPath)
        equal(c.variables.PanelHeight,'1000')
        ...
        equal(c.variables.PanelHeight,'972'); equal(c.persisted.CPUProcessCount,nil)
        ...
        equal(c.variables.PanelHeight,'916')
        c:call('Toggle','CPUShowInfo'); c:call('Toggle','CPUShowCores')
        equal(c.variables.PanelHeight,'888')
        for _, row in ipairs({'Info','Cores','Decimals','VoltageDecimals','TemperatureDecimals','Processes','History','Sensors','Temperature','Voltage','SensorHive','Reconnect','UpdateRate','SensorRate'}) do
