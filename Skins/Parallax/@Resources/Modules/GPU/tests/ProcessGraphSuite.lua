...
-- Fixtures expose only five counter ranks, validated name packets, time,
-- shared geometry and meter writes. No files or process actions are permitted.
local Suite = {}
    ...
    local function fixture(rows, options)
        ...
        function skin:Bang(command, ...)
            ...
            if command == '!SetOption' then
                ...
                assert(rank and (kind == 'Name' or kind == 'Value' or kind == 'Bar'), 'unexpected meter write')
        ...
        f.env = env
        function f:update() self.value, self.request = self.env.Update(); return self.value, self.request end
