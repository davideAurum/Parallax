...
foreach ($source in @('Core.lua', 'ConnectionCore.lua', 'Connection.lua', 'ConnectionMeters.inc', 'ConnectionMeasures.inc', 'Tests\ConnectionSuite.lua')) {
...
try {
    ...
    $skinDirectory = Join-Path $skinRoot 'NetworkConnectionTests'
...
function Initialize()
    local ok, report = pcall(function()
        ...
        local connection = dofile([=[$moduleRoot\ConnectionCore.lua]=])
        local suite = dofile([=[$testsRoot\ConnectionSuite.lua]=])
        return suite.run(core, connection, {moduleRoot = [=[$moduleRoot]=]})
    end)
...
"@
    ...
    Write-Output "Running production connection logic with installed Rainmeter $version in isolated temporary settings."
    ...
    Write-Output $report
    if ($report -notmatch 'SUMMARY: (\d+) passed, 0 failed') { throw 'Network connection tests failed; see report above.' }
}
finally {
