...
foreach ($source in @('Core.lua', 'Settings.lua', 'SettingsMeters.inc', 'Tests\SettingsSuite.lua')) {
...
try {
    ...
    $skinDirectory = Join-Path $skinRoot 'NetworkSettingsTests'
...
function Initialize()
    local ok, report = pcall(function()
        ...
        local suite = dofile([=[$testsRoot\SettingsSuite.lua]=])
...
"@
    ...
    Write-Output "Running production settings controller with installed Rainmeter $version in isolated temporary settings."
    ...
    Write-Output $report
    if ($report -notmatch 'SUMMARY: (\d+) passed, 0 failed') { throw 'Network settings tests failed; see report above.' }
}
finally {
