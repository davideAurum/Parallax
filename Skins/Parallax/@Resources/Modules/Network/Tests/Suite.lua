...
function Suite.run(Core, fixtures)
    ...
    test('controller sanitizes adapter labels and reports invalid fallback', function()
        local mock = controller()
        mock.sample.alias = 'NIC [!Quit] #CURRENTCONFIG#'
        mock:tick()
        truth(not mock.latest['MeterNetworkAdapter:Text']:find('[#%[%]]'))
    ...
    test('width action writes only Columns and refreshes current skin', function()
