...
Provider behavior and integration constraints are detailed in [PROVIDERS.md](PROVIDERS.md). Build and release tooling requirements are separate from utility runtime requirements; see [PACKAGING.md](PACKAGING.md). The Welcome panel's on-demand update check (WebParser, plus the one-shot `UpdateInstall.ps1` helper on Install) is optional and described in [UPDATES.md](UPDATES.md). Dependency tables should record only versions or constraints supported by the source and evidence, rather than guessing a minimum or claiming compatibility with an untested latest version.
